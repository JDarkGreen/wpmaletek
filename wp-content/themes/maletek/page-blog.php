...
<?php get_header(); ?>
		...
		<div class="container">
			<div class="row">
				<div class="col-xs-12">
					...
					<!-- menu Lista para filtrar los contenidos del blog -->
					<?php  
						/* Conseguimos las subcategorias de la categoria "blog" */
						$include = "Blog";
						$cat_base_id = '';
						...
						foreach ($categories as $cat ) {
							if ( $cat->name === $include ) {
								$cat_base_id = $cat->cat_ID;
							}
						}

						$blog_sub_categories = get_categories( 
							array( 
								'hide_empty' => false,
								'child_of'   => $cat_base_id,
								'orderby'    => 'id',
								'order'      => 'desc'
							) 
						);
					...
					<!-- Web -->
					<nav class="hidden-xs">
						<ul class="mnu-filter-blog list-inline text-uppercase text-center">
							<li><a href="#" data-category="last-news"> últimas <strong>novedades</strong></a></li>

							<?php foreach ( $blog_sub_categories as $blog_sc ) : ?>
							<li>
								<a href="#" data-category="<?php echo $blog_sc->name; ?>">
									<?php echo $blog_sc->name ?>
								</a>
							</li>
							<?php endforeach; ?>

							<li><a href="#" data-category="search"> buscador </a></li>
						</ul><!-- /mnu-filter-blog -->
					</nav> <!-- /nav -->

					<div class="container-search">
						<input type="text" id="search-blog" placeholder="Escribe aquí..." />
					</div> <!-- /.container-search -->
						
					<!-- Mobile -->
					<select name="sl-filter-blog" id="sl-filter-blog" class="visible-xs-block text-center center-block dropdown-red sl-filter-blog">
						<option value="last-news" selected> últimas <strong>novedades</strong></option>

						<?php foreach ( $blog_sub_categories as $blog_sc ) : ?>
							<option value="<?php echo $blog_sc->name ?>"><?php echo $blog_sc->name ?></option>
						<?php endforeach; ?>

					</select> <!-- /select sl-filter-blog -->

					<!-- Contenedor del contenido -->
					<section class="container-content-blog">
					</section> <!-- /container-content-blog -->
					
					<!-- Contenedor no contenido -->
					<section class="container-content-blog--no-info">
					</section> <!-- /container-content-blog--no-info -->

					<div id="js-loader-blog" class="js-loader-blog text-center">
	...
	<div class="center-block" style="height:60px"></div>
