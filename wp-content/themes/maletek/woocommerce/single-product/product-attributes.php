...
?>
<!-- modificando para nuestro tema para volver a su estado original descomentar -->
		...
		<div class="<?php if ( ( $alt = $alt * -1 ) == 1 ) echo 'alt'; ?>">
			<?php 
				...
				if ( $attribute['is_taxonomy'] ) {
					...
					$values = wc_get_product_terms( $product->id, $attribute['name'], array( 'fields' => 'names' ) );
					/* Remove wpautop esto crea <p></p> en los atributos*/
					/*$attribute_values = apply_filters( 'woocommerce_attribute', wpautop( wptexturize( implode( ', ', $values ) ) ), $attribute, $values );*/
					$attribute_values = apply_filters( 'woocommerce_attribute', wptexturize( implode( ', ', $values ) ) , $attribute, $values );
				} else {
			...
			<p><?php echo $attribute_name . " : " . $attribute_values; ?></p>
