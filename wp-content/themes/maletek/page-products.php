					...
					<?php if (have_posts()) : while(have_posts()) : the_post(); ?>
						...
						<div class="hidden-xs col-sm-6">
							...
							<article class="page-content">
								...
								<h2 class="title-product"><span><?php the_title(); ?></span></h2>

								<?php the_content(); ?>

								<?php
									$linkissuu = get_post_meta($post->ID, 'mb_issuu');
								?>
						...
						<div class="col-xs-12 col-sm-6">
							...
							<?php
								$arr_slides = array();
								$slide = get_post_meta($post->ID, 'slider-1-src');
								$title = get_post_meta($post->ID, 'slider-1-title');
								if (count($slide) > 0 && !empty($slide[0])) {
									$arr_slides[] = array(
										'src'	=>	$slide[0],
										'title' =>	$title[0]
									);
								...
								$slide = get_post_meta($post->ID, 'slider-2-src');
								$title = get_post_meta($post->ID, 'slider-2-title');
								if (count($slide) > 0 && !empty($slide[0])) {
									$arr_slides[] = array(
										'src'	=>	$slide[0],
										'title' =>	$title[0]
								...
								$slide = get_post_meta($post->ID, 'slider-3-src');
								$title = get_post_meta($post->ID, 'slider-3-title');
								if (count($slide) > 0 && !empty($slide[0])) {
									$arr_slides[] = array(
										'src'	=>	$slide[0],
										'title' =>	$title[0]
								...
								$slide = get_post_meta($post->ID, 'slider-4-src');
								$title = get_post_meta($post->ID, 'slider-4-title');
								if (count($slide) > 0 && !empty($slide[0])) {
									$arr_slides[] = array(
										'src'	=>	$slide[0],
										'title' =>	$title[0]
						...
						<?php
							if (count($arr_slides)) {
								$gallery_count = count($arr_slides);
						?>
							...
							<div id="carousel-products" class="carousel slide carousel-products" data-ride="carousel">
								<!-- Indicators -->
							  	...
							  	<?php for($i = 0; $i < $gallery_count; $i++) : ?>
							    	...
							    	<li data-target="#carousel-products" data-slide-to="<?php echo $i; ?>" <?php echo $active; ?>></li>
							    <?php endfor; ?>
							  	</ol>
							  	...
							  	<?php $i = 0; ?>
							  	<?php foreach($arr_slides as $gi) : ?>
							  		<?php $active = ($i == 0) ? 'active' : ''; ?>
							    	<div class="item <?php echo $active; ?>">
										<a class="fancybox" href="<?php echo $gi['src']; ?>" data-fancybox-group="gallery" title="<?php echo $gi['title']; ?>">
											<img src="<?php echo $gi['src']; ?>" class="img-responsive" alt="" />
											<div class="carousel-caption">
	    										<p><?php echo $gi['title']; ?></p>
	  										</div>
										</a>
								...
								<?php if (count($linkissuu) > 0 && $linkissuu[0] != '') : ?>
									...
									<div class="content-pdf">
										<figure>
											<a href="<?php echo $linkissuu[0]; ?>" target="_blank">
												<img src="<?php echo IMAGES; ?>/page_content_pdf.png" />
												<span>Ver catálogo de <br/> productos <strong><u>aquí</u></strong></span>
											</a>
										</figure>
									</div> <!-- /page-content-pdf -->
								<?php endif; ?>
