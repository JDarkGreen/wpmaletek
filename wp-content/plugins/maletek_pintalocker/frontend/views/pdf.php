...
$DB->select(WP_PREFIX . 'maletek_pintalocker_request', null, 'MD5(id)="'.$request.'"'); 
$reg=$DB->get_first('array');
...
if($reg['id']){
    ...
    #item
    $DB->select(WP_PREFIX . 'maletek_pintalocker_item', null, 'id="'.$reg['idItem'].'"' ); 
    $item=$DB->get_first('array');
    
    $dateCreate=strtotime($reg['dateCreate']);
    
    $content.= '<html><head>
        <title>Pinta tu Locker - Solicitud</title>
    ...
    $content.= '<div id="pageForm">';       
        $content.= '<div class="field">';
            $content.= '<div class="label">Fecha solicitud:</div>';
            $content.= '<div class="input">'.date('d/m/Y h:ia', $dateCreate ).'</div>';
        $content.= '</div>';
        $content.= '<div class="field">';
            $content.= '<div class="label">Nombres:</div>';
            $content.= '<div class="input">'.$reg['varName'].'</div>';
        $content.= '</div>';
        $content.= '<div class="field">';
            $content.= '<div class="label">Apellidos:</div>';
            $content.= '<div class="input">'.$reg['varLastName'].'</div>';
        $content.= '</div>';
        $content.= '<div class="field">';
            $content.= '<div class="label">Empresa:</div>';
            $content.= '<div class="input">'.($reg['varBusiness']?$reg['varBusiness']:'--').'</div>';
        $content.= '</div>';
        $content.= '<div class="field">';
            $content.= '<div class="label">Email:</div>';
            $content.= '<div class="input">'.$reg['varEmail'].'</div>';
        $content.= '</div>';
        $content.= '<div class="field">';
            $content.= '<div class="label">Tel&eacute;fono:</div>';
            $content.= '<div class="input">'.$reg['varPhone'].'</div>';
        $content.= '</div>';        
        $content.= '<div class="field">';
